...
def process_flags(cursor, results):
    ...
    if not results.result-id or results.result-id is " ":
        ...
        if not api.linkChk("https://refstack.openstack.org/api/v1/results/" + results.result-id):
            ...
            print("The result with an ID of " + results.result-id + " is not recorded in the database. Please add this result and try again.")
            sys.exit()
        else:
...
def create_license(cursor, db, _id, licenseLink, resultId, _type, _date): 
    if not dupChk("license", "id", _id):
        query = "INSERT INTO license(id, _link, result_id, _type, _date) VALUES('%s', '%s', '%s', '%s', '%s')" % (_id, licenseLink, resultId, _type, _date)
        cursor.execute(query)
        db.commit()
    else:
       print("The license associated with the license id " + _id + " already exists within the database.") 


def main():
    db, cursor, parser = api.connect()
    parser.add_argument("-l", "--license-link", type=str, action="store", required=True, help="License link")
    parser.add_argument("-t", "--type", type=str, action="store", help="product type")
    parser.add_argument("-d", "--date", type=str, action="store", required=True, help="License Date")
    parser.add_argument("-r", "--result-id", type=str, action="store", required=True, help="Passing result ID associated with this license")
