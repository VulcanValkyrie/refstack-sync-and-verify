...
def create_ticket(cursor, db, tik_link, tikId, productId):
    if not api.dupChk("ticket", "tik_link", tik_link, cursor):
        query = "INSERT INTO ticket(id, tik_link, product_id) VALUES('%s', '%s', '%d')" %  (tikId, tik_link, productId)
...
def main():
    ...
    parser.add_argument("-t", "--ticket", type=str, action="store", required=True, help="Ticket Link")
    parser.add_argument("-p", "--productName", type=str, action="store", required=True, help="Product Name")
